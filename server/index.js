...
export const registerUser = async (data: RegisterFormData) => {
  const { email, phone, username, fullName, password, role = 'employee' } = data;
  ...
  const { data: authData, error: authError } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        ...
        full_name: fullName,
        phone,
        role // <-- Set the role here (e.g., 'admin' or 'employee')
      }
    }
  });
...
export const verifyOTP = async (code: string, type: 'email' | 'phone') => {
  ...
  if (updateError) throw new Error('Failed to verify ' + type);
...
// --- Helper functions for OTPs (mock) ---
